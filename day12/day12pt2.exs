defmodule Day12 do
  ...
  def main do
    ...
    priorityQueue = Enum.reduce(0..length(matrixInput) - 1, %{}, fn row, acc ->
      Enum.reduce(0..length(Enum.at(matrixInput, 0)) - 1, acc, fn col, acc2 ->
        acc2
          |> Map.put(
            "#{col}-#{row}",
    ...
    Enum.reduce_while(1..20000, %{priorityQueue: priorityQueue, visitedNodes: []}, fn iteration, acc ->
      ...
      clampedTopDist = if topDist === :infinity do 999999999 else topDist end
      ...
      newAcc = %{
          ...
          if neighborDist === :infinity or neighborDist > clampedTopDist + 1 do
            filteredAcc2 = acc2 |> Enum.filter(fn {key, _} -> key != neighborKey end)
            newEntry = {"#{neighborKey}", 
              if neighborDist === :infinity do clampedTopDist + 1 else neighborDist end
            }
      ...
      #if no more nodes to visit, halt
      if length(newAcc.priorityQueue) == length(newAcc.visitedNodes) do
        IO.inspect("done")
        IO.inspect(newAcc)
        # IO.inspect(Enum.find(newAcc.priorityQueue, fn {key, _} -> key == "#{Enum.at(endCoords, 0)}-#{Enum.at(endCoords, 1)}" end) |> elem(1))

        #log the dist of the shortest path from startcoords to an "a" in the matrix
        IO.inspect(
          Enum.filter(newAcc.priorityQueue, fn {key, _} -> 
            coords = String.split(key, "-", trim: true) |> Enum.map(fn x -> String.to_integer(x) end)
            Enum.at(Enum.at(matrixInput, Enum.at(coords, 1)), Enum.at(coords, 0)) == "a"
          end)
