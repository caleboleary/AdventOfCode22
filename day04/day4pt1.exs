  ...
  def main do
    ...
    |> Enum.map(fn pair ->
      ...
      if (String.to_integer(Enum.at(one, 0)) <= String.to_integer(Enum.at(two, 0)) &&
            String.to_integer(Enum.at(one, 1)) >= String.to_integer(Enum.at(two, 1))) ||
           (String.to_integer(Enum.at(two, 0)) <= String.to_integer(Enum.at(one, 0)) &&
              String.to_integer(Enum.at(two, 1)) >= String.to_integer(Enum.at(one, 1))) do
        1
      else
        0
      end
