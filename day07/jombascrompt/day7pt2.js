...
const main = () => {
    ...
    const spaceToFree = 30000000 - (70000000 - sizes);
    console.log('spaceToFree', spaceToFree )
    console.log('sizes', sizes)

    const sortedDirSizes = allSizes.sort((a, b) => a - b);
    for (let i = 0; i < sortedDirSizes.length; i++) {
        const dirSize = sortedDirSizes[i];
        if (dirSize >= spaceToFree) {
            console.log('dirSize to delete', dirSize)
            break;
        }
    }
}
...
console.log('allSizes', allSizes.sort((a, b) => a - b))